...
public class JwtUtils {
    ...
    public static JwtInfo getMemberIdByJwtToken(HttpServletRequest request) {
        ...
        JwtInfo jwtInfo = new JwtInfo(claims.get("id").toString(), claims.get("username").toString(), claims.get("avatar").toString());
        return jwtInfo;
    }

    public static JwtInfo getMemberIdByJwtToken(String jwtToken) {
        if(StringUtils.isEmpty(jwtToken)) {
            return null;
        }
        Jws<Claims> claimsJws = Jwts.parser().setSigningKey(getKeyInstance()).parseClaimsJws(jwtToken);
